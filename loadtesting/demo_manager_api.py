...
class DemoManagerTestAppAPI(TaskSet):
    ...
    def login(self):
        with self.client.request(
            ...
            json={"email": "testuser@example.dev", "password": "Test1234567!"},
            catch_response=True,
        ) as response:
    ...
    def health_check(self):
        with self.client.request(
            method="GET", url="/ht", catch_response=True
        ) as response:
    ...
    def get_books_with_index(self):
        ...
        with self.client.request(
            ...
            if (
                self.assert_request_success(books_response, label="Get Books")
                and request_duration_sec > 40
            ):
                books_response.failure(
                    f"!!! Get Books - Request took too long! {request_duration_sec}"
                )


class BaseServerTestAppAPI(HttpUser):
